...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $stmt = $conn->prepare("SELECT id, name, position, age, birthdate, address, phone, email, img, img_type FROM officials_sk WHERE id = ?");
...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['img']['tmp_name'])) {
        ...
        $stmt = $conn->prepare("UPDATE officials_sk SET img = ?, img_type = ? WHERE id = ?");
