...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['img']['tmp_name'])) {
        ...
        $stmt = $conn->prepare("UPDATE officials_barangay SET img = ?, img_type = ? WHERE id = ?");
    ...
    $stmt1 = $conn->prepare("UPDATE officials_barangay SET name = ?, position = ?, age = ?, birthdate = ?, address = ?, phone = ?, email = ? WHERE id = ?");
